/*程序功能: 已知数据文件in.dat中存有200个四位数，并已调用函数readDat()把这些数存入数组a中。功能是:把个位数字和千位数字重新组成一个新的含有两位数字的数(新数的十位数字是原四位数的个位数字，新数的个位数字是原十位数的千位数字)，以及把百位数字和十位数字组成另一个新的含有两位数字的数(新数的十位数字是原四位数的百位数字，新数的个位数字是原四位数的十位数字），如果两个新组成的数均是偶数并且至少有一个数能被9整除。同时两个新数的十位数字均不为零，则将满足此条件的四位数按从大到小的顺序存入数组b中，并要计算满足上述条件的四位数的个数cnt.最后main()函数调用写函数writeDat()把结果cnt以及数组b中符合条件的四位数输出到out.dat文件中
 * 时间:
 * 作者:
 *
 *
