/*程序功能:已知数据文件in.dat中存有300个四位数，并已调用读函数readDat()把这些数存入数组a中。求出个位数上的数减千位数上的数减百位数上的数减十位数上的数大于零的个数cnt,再把满足此条件的四位数依次存入数组b中，然后对数组b的四位数按从大到小的顺序进行排序。最后main()函数调用写函数writeDat()把数组b中的数据输出到文件out.dat中 
 * 时间:
 * 作者:
 *
 *
