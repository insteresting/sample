/*程序功能: 已知数据文件in.dat中存有200个四位数，并已调用读函数readDat()把这些数存入数组a中。把一个四位数的千位数位置上的值加上个位数位置上的值恰好等于百位数位置上的值加上十位数位置上的值，并且原四位数是奇数，则统计出满足此条件的个数cnt，并把这些四位数按从小到大的顺序存入数组b中，最后调用写函数writeDat()把结果以及数组b中符合条件的四位数输出到out.dat文件中
 * 时间:
 * 作者:
 *
 *
