/*程序功能:把一个四位数的个位数位置上的值减去千位数位置上的值再减去百位数位置上的值最后减去十位数位置上的值，如果得出的值大于等于零且原四位数是偶数，则统计出满足此条件的个数cnt,并把这些数按从小到大的顺序存入数组b中，最后调用写函数writeDat()把结果cnt以及数组b中符合条件的四位数输出到out.dat文件中。
 * 时间:
 * 作者:
 *
 *
