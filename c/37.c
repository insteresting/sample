/*程序功能:求出千位数上的数加百位数上的数等于十位数上的数加个位数上的数的个数cnt,再求出所有满足此条件的四位数平均值pjz1，以及所有不满足此条件的四位数平均值pjz2。最后main()函数调用写函数writeDat()把结果cnt,pjz1,pjz2输出到out.dat文件中
 * 时间:
 * 作者:
 *
 *
...
void writeDat()
{
	...
	fprintf(fp,"%d\n%7.2lf\n%7.2lf\n",cnt,pjz1,pjz2); //把素数的个数写入到文件out.dat
